...
def load_cifar100(get_test_data=True, channels_last=True):
    ...
    directoryPath = os.path.join(BASE_DIR, 'cifar-100-python')
    images = None # No test data return
    fileName = 'train'
    fpath = os.path.join(directoryPath, fileName)
    with open(fpath, 'rb') as f:
        content = pickle.load(f, encoding='bytes')
    if images is None:
        images = content[b'data']
    ...
    with open(fpath, 'rb') as f:
        content = pickle.load(f, encoding='bytes')
    testImages = content[b'data'].reshape(-1, 3, 32, 32)
    testLabels = np.asarray(content[b'fine_labels'])

    # normalize data by dividing by 255:
    testImages = testImages / 255.
    # Make channels last: ==> NHWC
    if channels_last:
        testImages = np.moveaxis(testImages, 1, -1)

    return images, labels, testImages, testLabels


def stripConsts(graph_def, max_const_size=32):
    stripDefination = tf.GraphDef()
    for n0 in graph_def.node:
        n = stripDefination.node.add()
        n.MergeFrom(n0)
        if n.op == 'Const':
...
def show_graph(graph_def, max_const_size=32):
    ...
    stripDef = stripConsts(graph_def, max_const_size = max_const_size)
    ...
    """.format(data=repr(str(stripDef)), id='graph' + str(np.random.rand()))
